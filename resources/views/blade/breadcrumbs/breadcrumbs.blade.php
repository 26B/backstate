<div>
    <nav class="sm:hidden text-sm leading-5 font-medium">
        @if(count($breadcrumbs) > 1)
            ...
            >
                <backstate:icon name="solid.chevron-left" color="gray-400"/>
                {{ __('Back') }}
            </a>
        @else
            <backstate:breadcrumbs.element name="{{ $breadcrumbs[0]['name'] }}" isLast/>
        @endif
    </nav>
    <nav class="hidden sm:flex items-center text-sm leading-5 font-medium">
        @foreach($breadcrumbs as $breadcrumb)
            <backstate:breadcrumbs.element
            ...
            @if (!$loop->last)
            <backstate:icon name="solid.chevron-right" color="gray-400"/>
            @endif
        @endforeach
    </nav>
</div>
